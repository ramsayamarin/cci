...
function stripJsonComments(s) {
  let out = '';
  let i = 0, inStr = false, strCh = null;
  while (i < s.length) {
    const c = s[i], n = s[i+1];
    if (inStr) {
      out += c;
...
function readJsonc(p) {
  try { return JSON.parse(stripJsonComments(fs.readFileSync(p, 'utf8'))); } catch(e) { return null; }
}
function readText(p) {
