...
function scanCopilot(args) {
  ...
  const userSettings = sh.readJsonc(path.join(root, 'settings.json')) || {};
  const config = sh.readJsonc(path.join(root, 'config.json')) || {};
  const mcpFile = sh.readJsonc(path.join(root, 'mcp-config.json')) || {};
  const permsRaw = sh.readJsonc(path.join(root, 'permissions-config.json')) || {};
