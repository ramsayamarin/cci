...
function buildTree(D) {
  ...
  userPlugins.forEach(function(p) {
    ...
    var skills = pluginSkills.filter(function(s) { return s.pluginName === name; });
    var cmds = pluginCommands.filter(function(c) { return c.pluginName === name; });
    var agents = pluginAgentsData.filter(function(a) { return a.pluginName === name; });
    var hooks = pluginHooksData.filter(function(h) { return h.pluginName === name; });
    var child = { id:'plugin:'+name, icon:ICONS.plugin, label: name, type:'plugin', data: p, dimmed: p.enabled === false, children: [] };
  ...
  projPlugins.forEach(function(p) {
    var name = p.name || 'unknown';
    var pSkillsList = pluginSkills.filter(function(s) { return s.pluginName === name; });
    var pCmds = pluginCommands.filter(function(c) { return c.pluginName === name; });
    var pAgentsList = pluginAgentsData.filter(function(a) { return a.pluginName === name; });
    var pHooksList = pluginHooksData.filter(function(h) { return h.pluginName === name; });
    var child = { id:'plugin:p:'+name, icon:ICONS.plugin, label: name, type:'plugin', data: p, dimmed: p.enabled === false, children: [] };
