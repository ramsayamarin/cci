...
function buildTree(D) {
  ...
  // === USER (Global) ===
  var mode = D.mode || 'claude-code';
  var userLabel = mode === 'copilot-cli' ? 'User (Copilot CLI)' : 'User (Global)';
  var userNode = { id:'scope-user', icon:ICONS.user, label:userLabel, data:{ mode: mode }, children: [] };
  ...
  // Permissions (Copilot CLI only — per-folder tool approvals)
  var perms = D.permissions;
  if (perms && (perms.locations || []).length) {
    var permsNode = {
      id: 'user-permissions',
      icon: '\u{1F510}',
      label: 'Permissions',
      count: perms.locations.length,
      type: 'permissions',
      data: perms,
      children: perms.locations.map(function(loc, i) {
        return {
          id: 'perm:' + i,
          icon: '\u{1F4C2}',
          label: loc.folder + (loc.exists ? '' : ' (missing)'),
          count: (loc.approvals || []).length,
    ...
    userNode.children.push(permsNode);
  }
  ...
  tree.push(userNode);
  if (mode !== 'copilot-cli') tree.push(projNode);
